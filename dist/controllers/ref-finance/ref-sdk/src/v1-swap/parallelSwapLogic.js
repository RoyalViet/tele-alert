...
exports.formatPoolNew = formatPoolNew;
exports.solveForMuFloat = solveForMuFloat;
exports.calculate_dx_float = calculate_dx_float;
exports.calculate_dy_float = calculate_dy_float;
exports.calculateOptimalOutput = calculateOptimalOutput;
exports.reducePools = reducePools;
exports.checkIntegerSumOfAllocations = checkIntegerSumOfAllocations;
//@ts-nocheck
const big_js_1 = __importDefault(require("big.js"));
big_js_1.default.RM = 0;
big_js_1.default.DP = 40;
big_js_1.default.NE = -40;
big_js_1.default.PE = 40;
///////////////////////////////
...
function formatPoolNew(pool, inputToken, outputToken) {
    ...
    p["gamma_bps"] = new big_js_1.default(10000).minus(p.fee);
    p["x"] = x;
    p["y"] = y;
    return p;
}
...
function solveForMuFloat(pools, totalDeltaX, inputToken, outputToken) {
    if (pools.length > 0) {
        let numerator = new big_js_1.default(totalDeltaX);
        let denominator = new big_js_1.default(0);
        for (var i = 0; i < pools.length; i++) {
            let p = formatPoolNew(pools[i], inputToken, outputToken);
            let numAdd = new big_js_1.default(p.x).times(10000).div(p.gamma_bps);
            numerator = numerator.plus(numAdd);
            let denomAdd = new big_js_1.default(p.x).times(p.y).div(p.gamma_bps).sqrt().times(100);
            denominator = denominator.plus(denomAdd);
        }
        const mu = new big_js_1.default(numerator).div(denominator);
        return mu;
    }
    else {
...
function calculate_dx_float(mu, pool, inputToken, outputToken) {
    ...
    let radical = new big_js_1.default(p.x).times(p.y).div(p.gamma_bps);
    let dxFloat = new big_js_1.default(mu)
        .times(100)
        .times(radical.sqrt())
        .minus(new big_js_1.default(p.x).times(10000).div(p.gamma_bps));
    return dxFloat;
}
/** calculate_dy_float
...
function calculate_dy_float(dx_float, pool, inputToken, outputToken) {
    ...
    let p = formatPoolNew(pool, inputToken, outputToken);
    let dx = new big_js_1.default(dx_float);
    let denom = new big_js_1.default(10000).times(p.x).plus(new big_js_1.default(p.gamma_bps).times(dx));
    let numerator = new big_js_1.default(p.y).times(dx).times(p.gamma_bps);
...
function calculateOptimalOutput(pools, inputAmount, inputToken, outputToken) {
    ...
    for (var i = 0; i < pools.length; i++) {
        ...
        let dxInt = new big_js_1.default(dx).round();
        dxArray.push(dxInt);
    }
    if (negativeDxValsFlag) {
        dxArray = reducePools(pools, dxArray, inputAmount, inputToken, outputToken);
    }
    let dxArraySum = new big_js_1.default(0);
    for (var i = 0; i < dxArray.length; i++) {
        dxArraySum = dxArraySum.plus(dxArray[i]);
    }
    let tempAllocationsArray = [];
    for (var i = 0; i < dxArray.length; i++) {
        let ndx = new big_js_1.default(dxArray[i]).times(inputAmount).div(dxArraySum).round();
        tempAllocationsArray.push(ndx);
    }
    let allocationsArray = checkIntegerSumOfAllocations(tempAllocationsArray, inputAmount);
...
function checkIntegerSumOfAllocations(allocations, totalInput) {
    var totalInput = new big_js_1.default(totalInput);
    var allocations = allocations.map((item) => new big_js_1.default(item).round());
    let alloSum = allocations
        .map((item) => new big_js_1.default(item))
        .reduce((a, b) => a.plus(b), new big_js_1.default(0));
    let offset = totalInput.minus(alloSum);
    //get largest allocation.
    let currMax = new big_js_1.default(0);
    let currMaxInd = 0;
...
//# sourceMappingURL=parallelSwapLogic.js.map